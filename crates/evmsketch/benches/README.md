...
## Fixtures and Git LFS

All fixture files are stored in [Git LFS](https://git-lfs.com). If you cloned the repo before LFS was set up, or without LFS installed, run:

```sh
git lfs install   # one-time hook setup per machine
git lfs pull      # downloads any LFS objects already in the repo
```

`git lfs install` only needs to be run once per machine. After that, `git pull` and `git push` handle LFS objects automatically alongside normal commits.
...
# Regenerate the trace_parsing fixture (only needed when repinning the benchmark tx)
make fixture RPC_URL=https://your-sepolia-node

# Regenerate the replay fixture (only needed when repinning the benchmark tx)
