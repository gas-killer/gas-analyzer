        ...
        Err(_) => {
            eprintln!(
                "Skipping trace_parsing bench: fixture not found.\n\
                 Locally: `git lfs pull` to fetch from LFS, or `make fixture RPC_URL=<sepolia-node>` to regenerate.\n\
                 In CI: check LFS bandwidth quota — gas_estimation and replay will still run."
            );
            return;
        }
    };
    ...
    let trace: alloy::rpc::types::trace::geth::DefaultFrame =
        match serde_json::from_str(&trace_json) {
            ...
            Err(_) if trace_json.starts_with("version https://git-lfs.github.com") => {
                eprintln!(
                    "Skipping trace_parsing bench: fixture is an LFS pointer — run `git lfs pull`."
                );
                return;
            }
