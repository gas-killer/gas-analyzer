...
impl EvmSketchExecutorBuilder {
    ...
    /// [`with_chain_id`](Self::with_chain_id), then selects the matching
    /// `Genesis` (and, later, `EthSpec` in `sim_env`). Errors if the chain is
    /// neither mainnet nor Sepolia: silently defaulting to mainnet when pointed
    /// at a different network would yield wrong hardfork activation and corrupt
    /// gas estimation.
