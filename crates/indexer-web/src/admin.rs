...
/// Render a small subset of CommonMark to HTML for `{{ html|safe }}`.
///
/// Pulldown-cmark passes raw inline/block HTML in the source through
/// verbatim, and the model's input bundle contains attacker-influenceable
/// strings (contract names, event messages) — so `<script>`/`onerror`
/// markup could survive into the admin's browser. Downgrade every raw HTML
/// event to text so it renders escaped and inert; only markup generated by
/// the markdown renderer itself reaches the template. Link/image
/// destinations are renderer-generated too, so they are restricted to
/// http/https/mailto/relative — a steered model emitting
/// `[x](javascript:…)` or a `data:` URI must not become a live href.
fn render_markdown(src: &str) -> String {
    use pulldown_cmark::{Event, Options, Parser, Tag, TagEnd, html};
    let mut opts = Options::empty();
    opts.insert(Options::ENABLE_STRIKETHROUGH);
    // One entry per open link/image: whether its Start was dropped, so the
    // matching End is dropped too while the inner text still renders.
    let mut open_links: Vec<bool> = Vec::new();
    let mut open_images: Vec<bool> = Vec::new();
    let parser = Parser::new_ext(src, opts).filter_map(move |ev| match ev {
        ...
        Event::Start(Tag::Link {
            ...
            let strip = !is_safe_link_dest(&dest_url);
            open_links.push(strip);
            (!strip).then_some(Event::Start(Tag::Link {
                link_type,
                dest_url,
                title,
                id,
        ...
        ev @ Event::End(TagEnd::Link) => (!open_links.pop().unwrap_or(false)).then_some(ev),
        Event::Start(Tag::Image {
            ...
            let strip = !is_safe_link_dest(&dest_url);
            open_images.push(strip);
            (!strip).then_some(Event::Start(Tag::Image {
                link_type,
                dest_url,
                title,
                id,
        ...
        ev @ Event::End(TagEnd::Image) => (!open_images.pop().unwrap_or(false)).then_some(ev),
        other => Some(other),
    });
...
/// Scheme allowlist for markdown link/image destinations. Tabs and newlines
/// are removed before scheme extraction, mirroring the HTML URL parser, so
/// an entity-encoded `java&#9;script:` can't smuggle a scheme past the
/// check (pulldown-cmark decodes entities in destinations). A colon
/// appearing after `/`, `?` or `#` doesn't start a scheme, so such
/// destinations are relative — as is anything with no colon at all.
fn is_safe_link_dest(dest: &str) -> bool {
    let dest = dest.trim_matches(|c: char| c <= ' ');
...
mod tests {
    ...
    fn markdown_raw_html_is_escaped() {
        ...
        assert!(
            !out.contains("<img"),
            "inline html must not pass through: {out}"
        );
        assert!(
            !out.contains("<script"),
            "block html must not pass through: {out}"
        );
        assert!(
            out.contains("&lt;script"),
            "raw html should render escaped: {out}"
        );
        assert!(
            out.contains("<em>world</em>"),
            "markdown formatting still works: {out}"
        );
    }

    #[test]
    fn markdown_dangerous_link_schemes_are_stripped() {
        let out = render_markdown(
            "[x](javascript:alert(1)) ![y](data:text/html,pwn) \
             [t](java&#9;script:alert(2)) <javascript:alert(3)> \
             [ok](https://example.com) [rel](/docs) [mail](mailto:a@b.c)",
        );
        let lower = out.to_ascii_lowercase();
        assert_eq!(
            out.matches("<a href=").count(),
            3,
            "only the allowlisted anchors survive: {out}"
        );
        assert!(
            !out.contains("<img"),
            "data: image must be stripped entirely: {out}"
        );
        assert!(
            !lower.contains("href=\"javascript:") && !lower.contains("href=\"data:"),
            "dangerous schemes must not reach an href: {out}"
        );
        assert!(
            out.contains('x') && out.contains('y') && out.contains('t'),
            "inner text/alt text of stripped links is kept: {out}"
        );
        assert!(
            out.contains(r#"<a href="https://example.com">ok</a>"#),
            "https links survive: {out}"
        );
        assert!(
            out.contains(r#"<a href="/docs">rel</a>"#),
            "relative links survive: {out}"
        );
        assert!(
            out.contains(r#"<a href="mailto:a@b.c">mail</a>"#),
            "mailto links survive: {out}"
        );
    }
}
