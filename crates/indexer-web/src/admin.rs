...
///
/// The source may contain attacker-influenceable text (contract names, event
/// messages), so raw HTML events are downgraded to text and link/image
/// destinations are scheme-checked; only markup generated by the renderer
/// itself reaches the template.
...
/// True if `url` is a relative reference or uses an allowlisted scheme
/// (`http`, `https`, `mailto`). Tabs/newlines are stripped before the scheme
