...
fn test_wasm_analyze_trace_returns_jsvalue() {
    let result = analyze_trace(&valid_sstore_trace(), &test_estimator_address(), &test_caller_address(), None);
    assert!(
        result.is_ok(),
...
fn test_wasm_analyze_trace_invalid_json_returns_error() {
    let result = analyze_trace("bad json", &test_estimator_address(), &test_caller_address(), None);
    assert!(result.is_err());
}

#[wasm_bindgen_test]
fn test_wasm_analyze_trace_invalid_address_returns_error() {
    let result = analyze_trace(&valid_sstore_trace(), "not-an-address", &test_caller_address(), None);
    assert!(result.is_err());
}
...
fn test_wasm_analyze_trace_response_fields() {
    let result = analyze_trace(&valid_sstore_trace(), &test_estimator_address(), &test_caller_address(), None).unwrap();
